...
namespace IntoIT.GilCar.Interfaces.CustomersCarInstallations
{
    public class DAL
    {
        ...
        public List<string> getDistincedDocNums()
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("select distinct docnum from[SAP_CRM].[dbo].[ErpToCrm] where CRMImportDate is null and CRM_ERROR_MSG is null and convert(varchar, SAPExportDate, 111)  >=DATEADD(DAY, -0, convert(varchar, DATEADD(DAY, -20, GETDATE()), 111))", conn);

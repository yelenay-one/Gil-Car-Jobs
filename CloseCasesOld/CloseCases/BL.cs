        ...
        public void StartInterface(DateTime start)
        {
            ...
            if (cases != null)
            {
                foreach (DataRow row in cases.Rows)
                {
                    try
                    {
                        if (row["newcarid"] != null && row["scheduledstart"] != null)
                        {
                            ...
                            if (coll != null && coll.Entities.Count > 0)
                            {
                                ...
                                //Filter2.AddCondition("new_service_station_id", ConditionOperator.Equal, tlv);
                                //Filter2.AddCondition("new_service_station_id", ConditionOperator.Equal, haifa);
                                //query.Criteria.Filters.Add(Filter2);
                                //LinkEntity lk1 = new LinkEntity("incident", "serviceappointment", "incidentid", "regardingobjectid", JoinOperator.Inner);                                
                                //lk1.LinkCriteria.AddCondition("scheduledstart", ConditionOperator.GreaterEqual, scheduledstart);
                                //lk1.LinkCriteria.AddCondition("scheduledstart", ConditionOperator.LessThan, scheduledstartEnd);
                                ...
                                //    dal.UpdateError(string.Format("{0}. ", "docNum: " + docnum + " car: " + newcarIdError + " incident didnt fount "), docnum);
                        ...
                        {
                            int docnum = (int)row["docnum"];                        
                            dal.UpdateError("newcarid or scheduledstart where null ", docnum);
                            Errors++;
                        }
                    }
                    catch (Exception ex)
